...
type BatchCreateForwardRulesRequest struct {
	Rules []CreateForwardRuleRequest `json:"rules" binding:"required,min=1,max=100,dive"`
}
...
// BatchCreateRules handles POST /forward-rules/batch
func (h *Handler) BatchCreateRules(c *gin.Context) {
	var req BatchCreateForwardRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Warnw("invalid request body for batch create rules", "error", err, "ip", c.ClientIP())
		utils.ErrorResponseWithError(c, err)
		return
	}

	// Convert request to UseCase commands with partial failure mode for validation
	cmds := make([]usecases.CreateForwardRuleCommand, 0, len(req.Rules))
	...
	for i, r := range req.Rules {
		...
		if err := id.ValidatePrefix(r.AgentID, id.PrefixForwardAgent); err != nil {
			h.logger.Warnw("invalid agent_id format in batch create", "agent_id", r.AgentID, "index", i, "error", err)
		...
		if r.ExitAgentID != "" {
			if err := id.ValidatePrefix(r.ExitAgentID, id.PrefixForwardAgent); err != nil {
				h.logger.Warnw("invalid exit_agent_id format in batch create", "exit_agent_id", r.ExitAgentID, "index", i, "error", err)
				preValidationFailed = append(preValidationFailed, dto.BatchOperationErr{
					ID:     identifier,
		...
		if len(r.ChainAgentIDs) > 0 {
			...
			for j, chainAgentID := range r.ChainAgentIDs {
				if err := id.ValidatePrefix(chainAgentID, id.PrefixForwardAgent); err != nil {
					h.logger.Warnw("invalid chain_agent_id format in batch create", "chain_agent_id", chainAgentID, "index", i, "error", err)
					preValidationFailed = append(preValidationFailed, dto.BatchOperationErr{
						ID:     identifier,
		...
		if len(r.ChainPortConfig) > 0 {
			...
			for agentIDStr, port := range r.ChainPortConfig {
				if err := id.ValidatePrefix(agentIDStr, id.PrefixForwardAgent); err != nil {
					h.logger.Warnw("invalid agent_id in chain_port_config in batch create", "agent_id", agentIDStr, "index", i, "error", err)
					preValidationFailed = append(preValidationFailed, dto.BatchOperationErr{
						ID:     identifier,
		...
		if r.TargetNodeID != "" {
			if err := id.ValidatePrefix(r.TargetNodeID, id.PrefixNode); err != nil {
				h.logger.Warnw("invalid target_node_id format in batch create", "target_node_id", r.TargetNodeID, "index", i, "error", err)
				preValidationFailed = append(preValidationFailed, dto.BatchOperationErr{
					ID:     identifier,
		...
		cmds = append(cmds, usecases.CreateForwardRuleCommand{
	...
	ucResult, err := h.batchRuleUC.BatchCreate(c.Request.Context(), usecases.BatchCreateCommand{
		Rules:      cmds,
...
// BatchDeleteRules handles DELETE /forward-rules/batch
func (h *Handler) BatchDeleteRules(c *gin.Context) {
	var req BatchDeleteForwardRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Warnw("invalid request body for batch delete rules", "error", err, "ip", c.ClientIP())
		utils.ErrorResponseWithError(c, err)
		return
	}
...
// BatchToggleStatus handles PATCH /forward-rules/batch/status
func (h *Handler) BatchToggleStatus(c *gin.Context) {
	var req BatchToggleStatusRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Warnw("invalid request body for batch toggle status", "error", err, "ip", c.ClientIP())
		utils.ErrorResponseWithError(c, err)
		return
	}
...
// BatchUpdateRules handles PATCH /forward-rules/batch
func (h *Handler) BatchUpdateRules(c *gin.Context) {
	var req BatchUpdateForwardRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Warnw("invalid request body for batch update rules", "error", err, "ip", c.ClientIP())
		utils.ErrorResponseWithError(c, err)
		return
	}
	...
	for _, u := range req.Updates {
		...
		if err := id.ValidatePrefix(u.RuleID, id.PrefixForwardRule); err != nil {
			h.logger.Warnw("invalid rule_id format in batch update", "rule_id", u.RuleID, "error", err, "ip", c.ClientIP())
		...
		if u.AgentID != nil && *u.AgentID != "" {
			if err := id.ValidatePrefix(*u.AgentID, id.PrefixForwardAgent); err != nil {
				h.logger.Warnw("invalid agent_id format in batch update", "agent_id", *u.AgentID, "rule_id", u.RuleID, "error", err, "ip", c.ClientIP())
		...
		if u.ExitAgentID != nil && *u.ExitAgentID != "" {
			if err := id.ValidatePrefix(*u.ExitAgentID, id.PrefixForwardAgent); err != nil {
				h.logger.Warnw("invalid exit_agent_id format in batch update", "exit_agent_id", *u.ExitAgentID, "rule_id", u.RuleID, "error", err, "ip", c.ClientIP())
